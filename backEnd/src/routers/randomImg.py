...
async def main(dataset: str):
    try:
        ...
        if (dataset == 'brain-tumor'):
            ...
            sub_folder = ['glioma', 'meningioma', 'notumor', 'pituitary']

        elif (dataset == 'kidney-stone'):
            path = 'Kidney-Stone-Detection/contents/CT_images/Test'
            sub_folder = ['Normal', 'Stone']

        elif (dataset == 'skin-infection'):
            path = 'Skin-Infection-Classification/contents/Dataset/test'
            sub_folder = ['Acne and Rosacea Photos',
        ...
        for x in sub_folder:
            ...
            r = requests.get(
                f"https://api.github.com/repos/mrakesh0608/{path}/{x}")
...
async def main(dataset: str):
    try:
        ...
        return {
            "data": {
                "randomImgURL": random.choice(json_object),
                "dataset": dataset
            }
        }
    except Exception as e:
